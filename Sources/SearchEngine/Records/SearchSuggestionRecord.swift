...
/// SearchSuggestion을 SQLite suggestion 결과 계층에서 사용하는 내부 Record 표현입니다.
///
...
struct SearchSuggestionRecord: Equatable, Sendable {
    ...
    /// suggestion이 생성된 문서 영역입니다.
    let source: SearchSuggestionSource

    /// suggestion 선택 시 기대되는 동작 종류입니다.
    let kind: SearchSuggestionKind

    /// suggestion이 특정 문서와 연결될 때 사용하는 식별자입니다.
